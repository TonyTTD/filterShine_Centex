...
const Calendar = () => {
  ...
  const onDateSelect = (e) => {
    let fromDate =
      document.getElementById("calendar-select-from").value || selectDate[0];
    let toDate = document.getElementById("calendar-select-to").value;

    !isDateRange
      ? setSelectedDate([moment(fromDate).format("MM-DD-YYYY"), ""])
      : setSelectedDate([
  ...
  return (
    <div className="calendar">
      ...
      <input id="calendar-select-to" type="date" hidden></input>
      <br></br>
      <Button
        variant="contained"
        size="small"
      ...
      <FormGroup>
        <FormControlLabel
          control={<Checkbox onClick={(e) => enableToDate(e)} />}
          label="Enable Range"
        />
      </FormGroup>
    </div>
