...
import {
  ...
  requestModal,
  selectedService,
} from "../atom_selector/recoil.js";
import "./comp-styling/backlog.css";
import RequestFormModal from "./modal/RequestFormModal";
...
var Backlog = () => {
  ...
  let [useSelectedService, setSelectedService] =
  ...
  const columns = [
    ...
    },
    {
      ...
      width: 175,
      valueGetter: (param) =>
        `${moment(param.row.serviceon).format("dddd")} / ${moment(
          param.row.serviceon
        ).format("MM-DD-YY")}`,
    },
    {
  ...
  const sortLogsByDate = (logs) => {
    ...
    return copied.sort((a, b) => {
      return (
        moment(a.serviceon).format("ddd") - moment(b.serviceon).format("ddd")
      );
    });
  };
