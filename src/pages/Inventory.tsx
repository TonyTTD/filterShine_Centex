...
const Inventory = () => {
  ...
  const sendRequest = () => {
    axios
      .put(
        `${process.env.REACT_APP_API_URL}/filtershine/api/filter`,
        updateFilterCount
      )
      .then((data) => {
        console.log(data);
  ...
  const columns = [
    ...
    },
    {
      ...
      width: 100,
      valueGetter: (param) =>
        `${Math.round((param.row.installed / param.row.stock) * 100)}%`,
  ...
  return (
    <>
      <div
        className="inventory"
        style={{ height: 800, width: "50%", margin: "5%" }}
      >
        {" "}
        Total Inventory
        <DataGrid
        ...
        <List>
          {filters.map((filter) => {
            return (
              <ListItem
                key={filter.id}
                onClick={(e) => onInventoryEdit(filter)}
              >
                <ListItemAvatar>
                  <Avatar>
                    <CircularProgress
                      variant="determinate"
                      value={Math.round(
                        (filter.installed / filter.stock) * 100
                      )}
                    />
                    <Box
                      sx={{
                ...
                <ListItemText
                  primary={`${filter.type}`}
                  secondary={`Installed: ${filter.installed} Unused: ${
                    filter.stock - filter.installed
                  } Stock: ${filter.stock}`}
                />
              </ListItem>
            );
